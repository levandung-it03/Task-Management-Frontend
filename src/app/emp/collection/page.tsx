...
import { createCollection, updateCollection } from '../../../apis/create-collection.page.api';
import { DTO_CreateCollection, COLLECTION_STATUS, CollectionStatus, DTO_CollectionItem } from '../../../dtos/create-collection.page.dto';
import { SearchUserToAssign, AssignedUsers } from '../../../app-reused/create-task/task-creation-form/task-creation.form';
import './page.scss';
import { fetchCollectionData } from '../../../apis/emp.home.page.api';
...
export default function CollectionListPage() {
  ...
  function handleOpenUpdate(collection: DTO_CollectionItem) {
    setForm({ ...collection });
  ...
  function handleUpdate(e: React.FormEvent) {
    ...
    if (form) {
      ...
      updateCollection(id, { name, description, startDate, endDate, deadline, status: status as CollectionStatus }).then((updatedCollection) => {
        setCollections((prev) => prev.map((c) => (c.id === id ? updatedCollection : c)));
      });
    }
  }

  function handleClickAddLeader(collectionId: string) {
    setShowAddLeaderModal(collectionId);
  ...
  function handleClickViewLeader(collectionId: string) {
    setShowUpdateLeaderModal(collectionId);
    // Lấy leader đã add từ collection (giả sử có trường leaders hoặc assignedLeaders)
    const collection = collections.find(c => c.id === collectionId);
    // Giả sử collection.leaders là object { userId: { username, fullName, role } }
    setUpdateLeaderAssignedUsers(collection?.leaders || {});
    setUpdateLeaderAssignedUsersHist(collection?.leaders || {});
  }
      ...
      <div style={{ maxWidth: 1700, margin: '0 auto', background: 'var(--background)', borderRadius: 18, boxShadow: '0 2px 16px #0001', padding: 48 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 32, color: 'var(--main-green)' }}>Collection Information</span>
            </div>
            <div style={{ color: '#64748b', fontSize: 16,marginBottom: 24, marginTop: 2, fontStyle: 'italic', marginLeft: 40 }}>See full Collection information</div>
          </div>
          <button
            style={{
              background: '#166534',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {collections.map((collection) => (
            <div key={collection.id} style={{ position: 'relative', background: 'linear-gradient(110deg, #e6f4ec 0%, #b2dbc7 30%, #5ac8a1 50%, #b2dbc7 70%, #e6f4ec 100%)', border: '1px solid var(--main-green)', borderRadius: 16, padding: '15px 32px', display: 'flex', alignItems: 'center', gap: 18 }}>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 22, color: 'var(--main-green)' }}>{collection.name}</span>
              </div>
              <div style={{ minWidth: 180, fontSize: 16, color: 'var(--main-green)', fontWeight: 500, textAlign: 'right' }}>
                Deadline: <span style={{ fontWeight: 700, color: '#d32f2f' }}>{collection.deadline}</span>
              </div>
              <button
                ...
                onClick={() => handleClickAddLeader(collection.id)}
              >
                Add Leader
              </button>
              <button
              ...
              {openMenu === collection.id && (
                <div
                  ref={el => { menuRefs.current[collection.id] = el; }}
                  style={{
                    position: 'absolute',
                    top: 54,
                    right: 16,
                ...
                >
                  <div
                    ...
                    onClick={() => handleOpenUpdate(collection)}
                  ...
                  <div
                    ...
                    onClick={() => {
                      setOpenMenu(null);
                      handleClickViewLeader(collection.id);
                    }}
                    onMouseOver={e => (e.currentTarget.style.background = '#f3f4f6')}
                    onMouseOut={e => (e.currentTarget.style.background = 'transparent')}
                  >
        ...
        <div style={{
          ...
          <div ref={modalRef} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 20, color: '#166534' }}>Update Collection</span>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  >
                    ...
                    <select
                      value={form.status || ''}
                      onChange={e => setForm(f => f ? { ...f, status: e.target.value as CollectionStatus } : f)}
                      style={{ border: 'none', outline: 'none', fontSize: 15, padding: '12px 12px', width: '100%', background: 'transparent' }}
                      required
                    >
        ...
        <div style={{
          ...
          <div ref={createModalRef} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 20, color: '#166534' }}>Create Collection</span>
            ...
            <form
              onSubmit={async e => {
                e.preventDefault();
                setShowCreateModal(false);
                createCollection(createForm).then((newCollection) => {
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={e => setCreateForm(f => ({ ...f, status: e.target.value as CollectionStatus }))}
                      style={{ border: 'none', outline: 'none', fontSize: 15, padding: '12px 12px', width: '100%', background: 'transparent' }}
                      required
                    >
                      {COLLECTION_STATUS.map(status => (
